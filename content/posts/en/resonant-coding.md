...
[^skills]: This is the question that worries me the most. There are skills that are developed only through repeated practice: the ability to read someone else's code, to detect problematic patterns, to anticipate how something will fail before it fails. If we delegate that practice to the model, what happens to our ability to do it ourselves? Do we become more efficient or simply more dependent?[^dependencia] I don't have an answer, but I suspect the correct answer involves using the tools without stopping to exercise the muscles that the tools could atrophy.
